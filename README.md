...
```
### 3.2.3. Запустить тест в соседнем терминале
```
python tests/test_api.py
```
...
- Dockerfile доступен в `/docker/Dockerfile`, Docker-образ собран, протестирован и загружен на Docker Hub. Образ включает модель, код сервиса, все зависимости, веб-сервис запускается на порту 5000. Ссылка на Docker Hub в `# 6. Ссылка на Docker-образ в Docker Hub` в данном документе.
